...
def calculate_transport_cost(dist, mode, group_size):
    if mode == "Auto":
        # Determine how many autos are needed (Max 3 people per auto)
        num_autos = -(-group_size // 3)  # Ceiling division

        #  Define Fare Components
...
def suggest_trip(total_budget, group_size, transport_mode, preferred_type=""):
    # Predict match scores using your Logistic Regression model
    df_p['Match_Score'] = place_pipeline.predict_proba(X)[:, 1]

    # Calculate transport and total costs
    ...
    #  Handle empty results or general search
    if options.empty:
        return pd.DataFrame()

        #  Always return top 3 based on ML Match_Score
